...
defmodule Yubot.Controller.Action do
  ...
  # POST /api/action
  def create(conn) do
    RAc.insert(%{data: conn.request.body}, key(conn), group_id(conn))
    |> handle_with_201_json(conn)
  end

  # GET /api/action/:id
  def retrieve(conn) do
    RAc.retrieve(conn.request.path_matches.id, key(conn), group_id(conn))
    |> handle_with_200_json(conn)
  end

  # GET /api/action
  def retrieve_list(conn) do
    RAc.retrieve_list(%{}, key(conn), group_id(conn))
    |> handle_with_200_json(conn)
  end
  ...
  def update(conn) do
    ...
    |> R.bind(&RAc.update(%{data: &1}, conn.request.path_matches.id, key(conn), group_id(conn)))
    |> handle_with_200_json(conn)
  end

  # DELETE /api/action/:id
  def delete(conn) do
    RAc.delete(conn.request.path_matches.id, nil, key(conn), group_id(conn))
  ...
  defp fetch_auth(auth_id, conn), do: Yubot.Repo.Authentication.retrieve(auth_id, key(conn), group_id(conn))
end
